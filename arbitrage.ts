...
function initPrices(coins) {
  return coins.reduce(
    (a, coin) => ({
      ...a,
      [coin]: priceList.find((item) => item.symbol === coin).price,
    }),
    {},
  );
}
...
async function exchangeCoins(pair, from, quantity, price) {
  const side = from === pair[0] ? "SELL" : from === pair[1] ? "BUY" : null;
  ...
  console.log("exhcangeCoins", query);
  const response = await secureQuery("/api/v3/order/test", query);

  if (response.code < 0) {
    console.log("code < 0", response);
    await sleep(2);
...
function getFormattedQty(symbol, quantity) {
  const stepSize = exchangeList.symbols.find((item) => item.symbol === symbol)
    ?.filters.find((item) => item.filterType === "LOT_SIZE")?.stepSize;
...
function convertCoin(pair, myCoin, price, quantity) {
  ...
  if (side === "SELL") {
    ...
    return {
      query: `symbol=${pair[0]}${
        pair[1]
      }&side=SELL&type=MARKET&quantity=${quantity}`,
      coin: pair[1],
      pair,
      price,
      sellQty: quantity,
...
async function checkFlow(options) {
  ...
  for (let i = 0; i < 3; i++) {
    const coin = convertCoin(
      options.coins[i],
      currentCoin,
      options.prices[symbols[i]],
      currentQuantity,
    );
  ...
  if (queries.up.coin.quantity > 1000) {
  ...
  if (queries.down.coin.quantity > 1000) {
