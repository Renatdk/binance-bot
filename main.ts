...
async function checkFlows() {
  ...
  for (let i = 0; i < flowOptions.length; i++) {
    ...
    if (flows) {
      ...
      createOrders(flows, coin, quantity);
      return null;
    }
  }
  await sleep(2);
...
async function createOrder(flow, prevQty) {
  ...
  if (prevQty) {
    console.log(
      ...
      symbol,
      prevQty,
      getFormattedQty(symbol, prevQty),
    );

    prevQty = flow.side === "SELL"
      ? getFormattedQty(symbol, prevQty)
      : getFormattedQty(symbol, prevQty / flow.price);
  }

  let qty = prevQty ?? flow.sellQty ?? flow.quantity;
  qty = getFormattedQty(symbol, qty);
  let query = `symbol=${flow.pair[0]}${
    flow.pair[1]
  }&side=${flow.side}&type=MARKET&quantity=${qty}`;
  ...
  if (newOrder.code < 0) {
    ...
    sendMessage({
      chat_id: 195282026,
      text: `https://www.binance.com/ru/trade/${flow.pair[0]}_${
        flow.pair[1]
      }?theme=dark&type=spot`,
    });
    sendMessage();
    if (flow.side === "BUY") {
...
async function createOrders(flows, coin, quantity) {
  for (let i = 0; i < flows.length; i++) {
    const response = await exchangeCoins(
      flows[i].pair,
      coin,
      quantity,
      flows[i].price,
